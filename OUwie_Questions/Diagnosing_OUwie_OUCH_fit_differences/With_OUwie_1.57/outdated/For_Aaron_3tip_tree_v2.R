## This code is identical to For_Aaron_3tip_tree.R, except that it slightly alters the regime painting
## to *not* paint the branch leading to Sp 1 and 2
require(ouch)
...
## This was how the regime painting was constructed in For_Aaron_3tip_tree.R
## ou.reg <- paint(ou.tree, subtree=c('1'='1','2'='2'), branch=c('1'='1', '2'='2'))
## Here, I drop the 'branch' specification
...
## Okay, from Aaron I understand this now. The regime at the root is critical and must be taken into account, because regardless of what regime a tip ends up in, we assume it has spent an infinite amount of time in the root regime. So, for this case where the root is in regime 1, and Sp1 and Sp2 have spent some amount of time evolving under regime 2, the weight of regime 1 is:
exp.W[1:2,1] <- exp(-1) + exp(-1) * (exp(ou.tree@times[2])-exp(0))
## This can be understood in the following way: the infinite amount of time that these species spent in regime 1 is captured by exp(-1). From t=ou.tree@times[2], though, the influence of regime 1 wanes. This waning is captured by exp(-1) * (exp(ou.tree@times[2])-exp(0)).
## The weight for regime 2, on the other hand is just:
exp.W[1:2,2] <- exp(-1) * (exp(1)-exp(ou.tree@times[2]))
## reflecting the increasing influence of regime 2 since the regime switch

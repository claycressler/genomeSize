...
```{r OUwieFit}
set.seed(10001)
source("OUwie/R/OUwie.R")
source("OUwie/R/varcov.ou.R")
source("OUwie/R/weight.mat.R")
source("OUwie/R/check.identify.R")
ouwiefit <- OUwie(tree, trait, model="OUM", scaleHeight=TRUE, root.station=TRUE, quiet=TRUE)
ouwiefit
```
To fit an OU model to this data using $\texttt{ouch}$, I first need to convert the tree from 'phylo' format to 'ouchtree' format and then 'paint' the tree with the regime painting. 
Based on having done this before, I know that $\texttt{OUwie}$ paints the trees slightly differently, in that it assumes that regime switches occur halfway along branches. 
In order to accomodate this in $\texttt{ouch}$, I need to actually add a node to the tree that is the switchpoint.
...
## add in another node whose ancestor is the root, and whose descendent is node 32
tr <- rbind(tr, c("128","1",as.numeric(tr[32,3])/2,"2"))
tr[32,2] <- "128"
tr[,3] <- as.numeric(tr[,3])
ouch.tree <- with(tr,ouchtree(nodes=nodes,ancestors=ancestors,times=times,labels=labels))
regime1 <- paint(ouch.tree, subtree=c('1'=1,'128'=2), branch=c('1'=1))

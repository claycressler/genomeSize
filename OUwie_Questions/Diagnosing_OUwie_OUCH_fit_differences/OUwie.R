	...
	if(model == "OU1" | model == "OUM" | model == "OUMV" | model == "OUMA" | model == "OUMVA"){
		if(scaleHeight==TRUE){
			...
			phy$edge.length<-(phy$edge.length/d)
                ...
                ## Initialization (from phy.tools)
                ## C.mat is the variance-covariance matrix assuming Brownian motion trait evolution
		C.mat<-vcv.phylo(phy)
                ## estimate of alpha
		...
		if(model == "OU1" | model == "OUM"){
			...
			edges.tmp=edges
                ...
                ## but then they use the sigma estimate as the estimate of both alpha and sigma, which seems weird

...
To figure out how $\texttt{OUwie}$ computes the weight matrix I had to work directly through weight.mat.R, line by line. In doing so, I have found two likely errors, which are fully detailed in the all of the R code below. To summarize what I have found, however:

* The weight.mat.R function does not reset the variable 'oldregime' to the regime at the root inside the regime loop ("for (j in 1:k)"). Thus, at the end of the first loop, 'oldregime' may not be the same as the root regime. This alters the calculation of the weight on the branches descending from the root. This is why I observed that the weight of regime 2 for tips that are never in anything but regime 1 was non-zero.
  ...
  ## cycle through each regime
	for(j in 1:k){
	  ...
	  for(i in 1:length(edges[,1])){
	    if (i==1) print(paste("For regime", j, "the initial oldregime is", oldregime))
...
##                This version resets 'oldregime' to the root regime                                      ##
##           every time the loop calculates the weights for a new regime                                  ##
############################################################################################################
############################################################################################################
...
## COMPARE:
## you can see that there is only a single difference in the n.cov vector
which(n.cov!=n.cov2)
## this difference is on the branch from the root to the clade in regime 1
edges[edges[,3]==which(n.cov!=n.cov2),]
## this seemingly small error in n.cov makes a big difference in the weight matrix because now *all* of the tips in regime 1 have a positive weight on regime 2
W-W2 

## note also that the weights do not sum to one in the OUwie calculation, where the do in ouch
## weights for the tips in regime 1 all time
W2[1,]
## weights for the tips that switch regime 1 --> regime 2
W2[64,]

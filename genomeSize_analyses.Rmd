...
fitComparison <- function(data) {
  ...
  ##################################################################
  ## Pairwise comparision 2. mpdMpleth (OUM) versus metamorphosis ##
  ##################################################################
  
  ## Fit the metamorphosis (OUM) model to the oum.mpdMpleth data
  mpdMpleth.as.meta <- data[["oum.mpdMpleth.data"]]
  mpdMpleth.as.meta[,2] <- data[["oum.meta.data"]][,2]
  fit.oum.meta.to.oum.mpdMpleth.data <- OUwie(tree_metamorphosis, mpdMpleth.as.meta, model=c("OUM"),root.station=FALSE, scaleHeight=TRUE, shift.point=1, quiet=TRUE)
  
  ## Fit the mpdMpleth (OUM) model to the oum.meta data
  meta.as.mpdMpleth <- data[["oum.meta.data"]]
  meta.as.mpdMpleth[,2] <- data[["oum.mpdMpleth.data"]][,2]
  fit.oum.mpdMpleth.to.oum.meta.data <- OUwie(tree_mpdMpleth, meta.as.mpdMpleth, model=c("OUM"),root.station=FALSE, scaleHeight=TRUE, shift.point=1, quiet=TRUE)
  ...
  fit.oum.mpdMpleth.to.oum.meta.data <- OUwie(tree_mpdMpleth, meta.as.mpdMpleth, model=c("OUMV"),root.station=FALSE, scaleHeight=TRUE, shift.point=1, quiet=TRUE)
  ...
  list(## Comparison #1
       ...
       ## Comparison #2
       fit.oum.meta.to.oum.mpdMpleth.data=fit.oum.meta.to.oum.mpdMpleth.data,
       fit.oum.mpdMpleth.to.oum.meta.data=fit.oum.mpdMpleth.to.oum.meta.data,
...
fitComparison2 <- function(data) {
  ...
  ##################################################################
  ## Pairwise comparision 1. mpdMpleth (OUM) versus metamorphosis ##
  ##################################################################
  ...
  list(fit.oum.meta.to.oum.mpdMpleth.data=fit.oum.meta.to.oum.mpdMpleth.data,
       fit.oum.mpdMpleth.to.oum.meta.data=fit.oum.mpdMpleth.to.oum.meta.data,
       fit.oum.meta.to.oum.meta.data=fit.oum.meta.to.oum.meta.data)
}
...
PMCfits3 <- PMCfits
for (i in 1:500) {
  for (j in 1:length(PMCfits2[[i]])) {
    PMCfits3[[i]][[names(PMCfits2[[i]])[j]]] <- PMCfits2[[i]][[j]]
  }
}
...
```
```{r pmcResults, echo=FALSE, fig.height=7, fig.width=6, fig.cap="Comparing the distributions of the test statistic when the data is generated by the non-adaptive model (bm1) or the adaptive model (oumv.mpdMpleth). The dashed line gives the actual value of the test statistic for the true data."}
par(mar=c(5,5,1,1), oma=rep(0,4), mfrow=c(2,2))
...
We can repeat these analyses to look at the p-value and the power for the second comparison, between the OUMV and OUM models fit to the mpdMpleth hypothesis. Again, we see a fairly low p-value (0.046) and a fairly high power (0.8), although there is clearly more overlap between the two distributions  (Fig. \@ref(fig:pmcResults)D).
...
We can also examine the p-value and the power for the third comparison, which determines how much support there is for metamorphosing plethodontids being a unique selective regime. Again, we see a fairly low p-value (0.038) and a fairly high power (0.81), although there is clearly more overlap between the two distributions (Fig. \@ref(fig:pmcResults)C).
...
Finally, we can examine the p-value and the power for the fourth comparison, which determines how much support there is distinguishing among different metamorphosis strategies. Again, we see a fairly low p-value (0.038) and a fairly high power (0.87), and there is less overlap between the two distributions  (Fig. \@ref(fig:pmcResults)C).

...
This also appears to be true of genome size in the metamorphosing non-plethodontid salamander regime (although since the peak of the genome size distribution is slightly smaller than the predicted mean, there are still some salamanders potentially experiencing some directional selection towards increasing genome sizes).
Genome size in the the metamorphosing plethodontid regime, on the other hand, appears to be under directional selection towards smaller genome sizes, since almost all metamorphosing plethodontid salamanders have genome sizes that are larger than the expected mean.
...
for (i in 1:4) {
  ...
  hist(tipdata$genomesize[tipdata$metap.np.dd.paed==hyp], freq=FALSE, main="", xlab="", ylab="", xlim=range(xseq), ylim=c(0,max(max(y$density),dnorm(mu,mean=mu,sd=sigma))))
...
data.frame(parameter=c('alpha','sigma.sq (DD)', 'sigma.sq (Meta-NP)', 'sigma.sq (Meta-P)', 'sigma.sq (Paed)', 'theta (DD)', 'theta (Meta-NP)', 'theta (Meta-P)', 'theta (Paed)'),
...
Using the same methodology, we can calculate the approximate p-value and power for the third test, $meta_p-meta_{np}-other$ (OUMV) and $metamorphosis-other$ (OUM). This tests whether distinguishing between different metamorphosis strategies is supported. Here the p-value is 0.084 and the power is 0.522. See Fig. \@ref(fig:pmcResults)C for a visual depiction of these results.


```{r, echo=FALSE}
delta <- -2*(results[["meta.other.OUMV"]]$loglik - results[["metap.np.other.OUMV"]]$loglik)
...
## p-value
metap.np.other.v.meta.other.p <- sum(delta < delta.dist)/length(delta.dist)
## power
metap.np.other.v.meta.other.power <- sum(delta.dist.2 > sort(delta.dist)[475])/length(delta.dist.2)
metap.np.other.v.meta.other.p
metap.np.other.v.meta.other.power
```


We also calculate the p-value and power for the fourth test, $meta_p-meta_{np}-paed-dd$ (OUMV) versus $meta-paed-dd$ (OUMV). This tests whether separate regimes for feeding and non-feeding metamorphosis are supported, after accounting for the two non-metamorphosing strategies. The more complicated model is strongly supported, with a p-value of 0.046 and power of 0.718. See Fig. \@ref(fig:pmcResults)D for a visual depiction of these results.

```{r, echo=FALSE}
delta <- -2*(results[["meta.dd.paed.OUMV"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik)
...
## p-value
metap.np.dd.paed.v.meta.dd.paed.p <- sum(delta < delta.dist)/length(delta.dist)
## power
metap.np.dd.paed.v.meta.dd.paed.power <- sum(delta.dist.2 > sort(delta.dist)[475])/length(delta.dist.2)
metap.np.dd.paed.v.meta.dd.paed.p
metap.np.dd.paed.v.meta.dd.paed.power
```

We also calculate the p-value and power for the fifth test, $meta_p-meta_{np}-paed-dd$ (OUMV) versus $meta_p-meta_{np}-other$ (OUMV). This tests whether allowing separate regimes for direct developers and paedomorphs is supported, once you have accounted for feeding and non-feeding metamorphosis. For this comparison, the p-value is 0.054 and the power is 0.728, indicating that there is fairly strong support for the more complex model. See Fig. \@ref(fig:pmcResults)E for a visual depiction of these results.

```{r, echo=FALSE}
delta <- -2*(results[["metap.np.other.OUMV"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik)
...
## p-value
metap.np.dd.paed.v.metap.np.other.p <- sum(delta < delta.dist)/length(delta.dist)
## power
metap.np.dd.paed.v.metap.np.other.power <- sum(delta.dist.2 > sort(delta.dist)[475])/length(delta.dist.2)
metap.np.dd.paed.v.metap.np.other.p
metap.np.dd.paed.v.metap.np.other.power
```


Finally, just to cross all the t's and dot all the i's, we can calculate the p-value and power for the sixth test, $meta_p-meta_{np}-paed-dd$ (OUMV) versus $meta_p-meta_{np}-paed-dd$ (OUM), which tests whether multiple $\sigma$ values is well-supported by the data. Here the p-value is 0.002 and the power is 0.96, suggesting very strong support for the more complex model. See Fig. \@ref(fig:pmcResults)F for a visual depiction of these results.

```{r, echo=FALSE}
delta <- -2*(results[["metap.np.dd.paed.OUM"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik)
...
## p-value
metap.np.dd.paed.oumv.v.metap.np.dd.paed.oum.p <- sum(delta < delta.dist)/length(delta.dist)
## power
metap.np.dd.paed.oumv.v.metap.np.dd.paed.oum.power  <-sum(delta.dist.2 > sort(delta.dist)[475])/length(delta.dist.2)
metap.np.dd.paed.oumv.v.metap.np.dd.paed.oum.p
metap.np.dd.paed.oumv.v.metap.np.dd.paed.oum.power
```
...
deltas <- c(-2*(results[["BM1"]]$loglik - results[["meta.other.OUMV"]]$loglik),
           ...
           -2*(results[["meta.other.OUMV"]]$loglik - results[["metap.np.other.OUMV"]]$loglik),
           -2*(results[["meta.dd.paed.OUMV"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik),
           -2*(results[["metap.np.other.OUMV"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik),
           -2*(results[["metap.np.dd.paed.OUM"]]$loglik - results[["metap.np.dd.paed.OUMV"]]$loglik))
...
complexModel <- c("meta-other","meta-dd-paed","metap-np-other","metap-np-dd-paed","metap-np-dd-paed","metap-np-dd-paed (OUMV)")
simpleModel <- c("BM","meta-other","meta-other","meta-dd-paed","metap-np-other","metap-np-dd-paed (OUM)")
